...
def home():
    if request.method == 'POST':
        ...
        texts = [doc.split() for doc in newsgroups_train.data]
        
        # spaCy model
        nlp = spacy.load('en_core_web_sm')

        # tokenization
        texts = [nlp(' '.join(doc)) for doc in texts]

        # stop words, lemmatization, punctuation, numbers
        texts = [[token.lemma_ for token in doc if not token.is_stop and not token.is_punct and not token.like_num] for doc in texts]
